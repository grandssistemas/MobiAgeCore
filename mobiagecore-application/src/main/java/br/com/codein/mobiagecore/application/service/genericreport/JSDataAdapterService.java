...
import br.com.codein.mobiagecore.domain.exceptions.ReportException;
...
public class JSDataAdapterService {
    ...
    private final List<String> USERS_KEYS = Arrays.asList(
    ...
    private String connect(JSONObject command) throws SQLException {
        Connection con = null;
        try {
    ...
    private String addFilterQuery(String queryString) {
        if (StringUtils.containsIgnoreCase(queryString, "INFORMATION_SCHEMA")) {
            ...
            if (StringUtils.containsIgnoreCase(queryString, "obj")) {
                return oorganizationFilterQuery(queryString);
            } else {
                throw new ReportException("report01;;That SQL must contains on the primary table the alias obj");
            }
        }
    }
    ...
    private String oorganizationFilterQuery(String queryString) {
        if (StringUtils.containsIgnoreCase(queryString, "WHERE")) {
            ...
            if (StringUtils.containsIgnoreCase(queryString, "GROUP BY")) {
    ...
    private StringBuilder mountConnection(StringBuilder command) {
        ...
        conn.replaceFirst("%address", getProperties().getProperty("database.url"));
        conn.replaceFirst("%db", getProperties().getProperty("database.name"));
        conn.replaceFirst("%schema", getProperties().getProperty("schema.name"));
        conn.replaceFirst("%user", getProperties().getProperty("database.user"));
        conn.replaceFirst("%pass", getProperties().getProperty("database.password"));
